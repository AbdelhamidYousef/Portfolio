 ...
 * <IconCardList
 *   list={skills}
 *   containerClassName="min-h-244 sm:min-h-136 2xl:min-h-120"
 *   itemClassName="group relative h-16 xl:h-20 rounded-xl border border-gray-100 shadow-xs overflow-hidden flex items-center hover:scale-105 transition-all duration-300 dark:bg-slate-700 dark:border-gray-600"
 * />
 */
const IconCardList = ({
  list,
      ...
      <AnimatePresence>
        {list.map((item, index) => {
          if (!hasIcon(item.iconName)) {
            console.error(
              `IconCardList: Icon for [${item.iconName}] not found`
            );
            return null;
          }
          ...
          return (
            <m.li
              key={item.id}
              variants={fadeIn('left', 'spring', 1, 0.1 * index)}
              initial="hidden"
              animate="show"
              exit="hidden"
            >
              <div
                className={`
                  ${itemClassName}
                  group relative h-16 xl:h-20 rounded-xl border border-gray-100 shadow-xs overflow-hidden flex items-center hover:scale-105 transition-all duration-300 dark:bg-slate-700 dark:border-gray-600
                `}
              >
